...
$data_array = array();
$data_array['$awards']=$plugin_language['awards'];
...
$ergebnis = safe_query("SELECT * FROM `" . PREFIX . "squads` WHERE
...
if ($db[ 'icon' ]) {
                $icon = '<a href="index.php?site=squads&amp;action=show&amp;squadID=' . $ds[ 'squadID' ] .
                    '"><img class="card-img-top" src="../images/squadicons/' . $db[ 'icon' ] . '" alt="' . getsquadname($ds[ 'squadID' ]) .
                    '"></a>';
            } else {
                $icon = '';
            }
    ...
    $data_array = array();
    if($ds['homepage'] != '') $data_array['$homepage'] = '<a href="'.$ds['homepage'].'" target="_blank"><i class="fas fa-globe-europe fa-3x fa-globe-europe-color-award"></i></a>';
        else $data_array['$homepage'] = '';
        if($ds['facebook'] != '') $data_array['$facebook'] = '<a href="'.$ds['facebook'].'" target="_blank"><i class="fab fa-facebook fa-3x fa-facebook-color-award"></i></a>';
        else $data_array['$facebook'] = '';
        if($ds['twitter'] != '') $data_array['$twitter'] = '<a href="'.$ds['twitter'].'" target="_blank"><i class="fab fa-twitter fa-3x fa-twitter-color-award"></i></a>';
        else $data_array['$twitter'] = '';
        if($ds['liga'] != '') $data_array['$liga'] = '<a href="'.$ds['liga'].'" target="_blank"><i class="fas fa-gamepad fa-3x fa-liga-color-award"></i></a>';
        else $data_array['$liga'] = '';
        if($ds['steam'] != '') $data_array['$steam'] = '<a href="'.$ds['steam'].'" target="_blank"><i class="fab fa-steam fa-3x fa-steam-color-award"></i></a>';
        else $data_array['$steam'] = '';
        if($ds['twitch'] != '') $data_array['$twitch'] = '<a href="'.$ds['twitch'].'" target="_blank"><i class="fab fa-twitch fa-3x fab-twitch-color-award"></i></a>';
        else $data_array['$twitch'] = '';
        if($ds['youtube'] != '') $data_array['$youtube'] = '<a href="'.$ds['youtube'].'" target="_blank"><i class="fab fa-youtube fa-3x fa-youtube-play-color-award"></i></a>';
        else $data_array['$youtube'] = '';
        if($ds['instagram'] != '') $data_array['$instagram'] = '<a href="'.$ds['instagram'].'" target="_blank"><i class="fab fa-instagram fa-3x fa-instagram-color-award"></i></a>';
    ...
    if ($gesamt) {
        if ($type == "ASC") {
            echo '<a href="index.php?site=awards&amp;page=' . $page . '&amp;sort=' . $sort . '&amp;type=DESC">' .
                $plugin_language[ 'sort' ] . ':</a> <i class="fa fa-chevron-down"></i>';
        } else {
            echo '<a href="index.php?site=awards&amp;page=' . $page . '&amp;sort=' . $sort . '&amp;type=ASC">' .
                $plugin_language[ 'sort' ] . ':</a> <i class="fa fa-chevron-up"></i>';
        }
